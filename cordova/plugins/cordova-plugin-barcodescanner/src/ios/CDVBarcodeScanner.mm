...
@property (nonatomic, retain) IBOutlet QRView* overlayView;
...
- (void)encode:(CDVInvokedUrlCommand*)command {
    ...
    processor.imagePath = command.arguments[0][@"url"];
    [processor retain];
    [processor retain];
    [processor retain];
...
- (void)scanBarcode {
    ...
    self.viewController.successblock = ^(NSString *code) {
        
        [self barcodeScanSucceeded:code format:@"QR_CODE"];
...
- (void)generateImage{
    ...
    //UIImage *centerImg=[UIImage imageNamed:self.imagePath];
    UIImage *centerImg=[[UIImage alloc] initWithContentsOfFile:self.imagePath];
    if (centerImg) {
        centerImg= [self drawCircleImage:centerImg];
        CGFloat centerW=width/4.5;
        CGFloat centerH=height/4.5;
...
- (UIImage *)drawCircleImage:(UIImage *)image {
    ...
    UIGraphicsBeginImageContextWithOptions(CGSizeMake(side, side), false, 4.0);
    float linewidth = side/10.0;
    UIBezierPath *zierPath = [UIBezierPath bezierPathWithRoundedRect:CGRectMake(0, 0, side, side) cornerRadius:linewidth];
    //zierPath.lineWidth = linewidth;
    //[[UIColor whiteColor] setStroke];
    //[zierPath stroke];
    [[UIColor whiteColor] setFill];
    [zierPath fill];
    
    CGContextAddPath(UIGraphicsGetCurrentContext(),[UIBezierPath bezierPathWithRoundedRect:CGRectMake(linewidth, linewidth, side-linewidth*2, side-linewidth*2) cornerRadius:linewidth].CGPath
                     );
    
    //[UIBezierPath bezierPathWithOvalInRect:CGRectMake(0, 0, side, side) cornerRadius:10.0].CGPath
...
- (void)viewDidLoad {
    ...
    _overlayView = [[QRView alloc]initWithFrame:self.view.bounds];
    _overlayView.transparentArea = CGSizeMake(300, 300);
    _overlayView.backgroundColor = [UIColor clearColor];
    [self.view addSubview:_overlayView];
    
    UIView*headerview = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.view.bounds.size.width, 64)];
    
    headerview.backgroundColor = [UIColor colorWithRed:40/255.0 green:40/255.0 blue:40/255.0 alpha:1];
    [self.view addSubview:headerview];
    
    UIButton *backbutton = [[UIButton alloc] initWithFrame:CGRectMake(16, 28, 24, 24)];
    // backbutton.backgroundColor = [UIColor redColor];
    
    [backbutton setBackgroundImage:[UIImage imageNamed:@"qr_scan_back.png"] forState:UIControlStateNormal];
    [backbutton addTarget:self action:@selector(cancelButtonPressed:) forControlEvents:UIControlEventTouchUpInside];
    [headerview addSubview:backbutton];
    
    UIButton *xiangcebutton = [[UIButton alloc] initWithFrame:CGRectMake(headerview.frame.size.width-48, 26, 40, 30)];
    //xiangcebutton.backgroundColor = [UIColor redColor];
    ...
    [headerview addSubview:xiangcebutton];
    
    self.failbutton = [[UIButton alloc] initWithFrame:_overlayView.bounds];
    self.failbutton.backgroundColor = [UIColor colorWithRed:40/255.0 green:40/255.0 blue:40/255.0 alpha:0.5];
    [self.failbutton setTitle:@"识别失败，轻触继续" forState:UIControlStateNormal];
    [self.failbutton  addTarget:self action:@selector(failbuttonButtonClick:) forControlEvents:UIControlEventTouchDragInside];
    self.failbutton.hidden = YES;
    [_overlayView addSubview:self.failbutton];
}
- (void)failbuttonButtonClick:(UIButton *)but{
    self.failbutton.hidden = YES;
    // [self.processor.captureSession startRunning];
}
-(void)xiangCeClick:(UIButton *)butt{
    [self.processor.captureSession stopRunning];
    QRImagePicker *impick = [[QRImagePicker alloc] init];
    CDVInvokedUrlCommand * command = [[CDVInvokedUrlCommand alloc] initWithArguments:@[@{@"maximumImagesCount" : @1, @"width" : @1920, @"height" : @1440, @"quality" : @100}] callbackId:nil className:@"ImagePicker" methodName:@"getPictures"];
    [impick pluginInitialize];
    impick.columnNumberTF = 3;
    impick.enableShowTakePhoto =NO;
    impick.viewController =self; //[UIApplication sharedApplication].windows[0].rootViewController;
    [impick getPictures:command];
    impick.cancelblock = ^{
        [self.processor.captureSession startRunning];
    };
    // __weak typeof(self) weakSelf = self;
    impick.successblock = ^(NSArray *patharray){
        // NSLog(@"successblock   %@",patharray);
        UIImage *image = [[UIImage alloc] initWithContentsOfFile:patharray[0]];
        ...
        if(features.count>0){
            ...
            //NSLog(@"successblock scannedResult  %@",scannedResult);
            if(scannedResult.length>0){
                self.successblock(scannedResult);
            }else{
